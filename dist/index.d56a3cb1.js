...
fetch(api_url).then((response)=>{
    ...
    worldMap.src = new URL(require("f1b8f24e0d2e3ed9"));
    ...
    const displayCountry = (countrys)=>{
        if (countrys.length !== 0) {
            ...
            if (lastSearch !== document.getElementById("cSearch").value) countryList.innerHTML = countrys.map((countrys)=>{
                countrys.currency = "";
                if (countrys.name !== "Antarctica") {
                    for(let i = 0; i < countrys.currencies.length; i++)countrys.currency = countrys.currency + countrys.currencies[i].name + " and ";
                    countrys.currency = countrys.currency.substring(0, countrys.currency.length - 4);
                } else countrys.currency = "Chickens";
                return `
            <li class="country">
...
},{"f1b8f24e0d2e3ed9":"7mGnI"}],"7mGnI":[function(require,module,exports) {
module.exports = require('./helpers/bundle-url').getBundleURL('fBg3F') + "world_map.0532bf85.png" + "?" + Date.now();
